...
// NOTE: parseFrontmatter is now in js/utils.js as window.siteUtils.parseFrontmatter
...
async function fetchMarkdownFile(path) {
    ...
    const { frontmatter, body } = window.siteUtils.parseFrontmatter(text);
        ...
        const posts = await Promise.all(
            files.map(async (filename) => {
                ...
                try {
                    ...
                    const { frontmatter, body, html } = window.siteUtils.parseFrontmatter(fullMarkdown);
...
async function loadSingleContent(collectionName, slug) {
    ...
    const fullMarkdown = await response.text();
    const { frontmatter, body } = window.siteUtils.parseFrontmatter(fullMarkdown);
    const html = markdownToHtml(body);
